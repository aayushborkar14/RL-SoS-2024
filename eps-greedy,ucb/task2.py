...
if __name__ == "__main__":
    ...
    # Part A
    task2p2s = [i * 0.05 for i in range(19)]
    task2p1s = [0.9] * 19
    regrets = task2(UCB, 30000, task2p1s, task2p2s, 1)
    print(regrets)
    plt.plot(task2p2s, regrets)
    plt.title("Regret vs p2")
    plt.savefig("task2A-{}-{}.png".format("UCB", time.strftime("%Y%m%d-%H%M%S")))
    plt.clf()

    # Part B
    task2p2s = [i * 0.05 for i in range(19)]
    task2p1s = [i + 0.1 for i in task2p2s]
    # UCB
    regrets = task2(UCB, 30000, task2p1s, task2p2s, 1)
    print(regrets)
    plt.plot(task2p2s, regrets)
    ...
    plt.clf()
    # KL_UCB
    regrets = task2(KL_UCB, 30000, task2p1s, task2p2s, 1)
    print(regrets)
    plt.plot(task2p2s, regrets)
    plt.title("Regret vs p2")
    plt.savefig("task2B-{}-{}.png".format("KL_UCB", time.strftime("%Y%m%d-%H%M%S")))
    plt.clf()
