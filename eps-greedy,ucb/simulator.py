...
def single_sim_faulty(
    seed=0, ALGO=FaultyBanditsAlgo, PROBS=[0.3, 0.5, 0.7], FAULT=0.2, HORIZON=1000
):
    np.random.seed(seed)
    np.random.shuffle(PROBS)
...
def single_sim_multi(
    seed=0, ALGO=MultiBanditsAlgo, PROBS=[[0.3, 0.5, 0.7]] * 2, HORIZON=1000
):
    np.random.seed(seed)
    ...
    for t in range(HORIZON):
        ...
        algo_inst.get_reward(
            arm_index=arm_to_be_pulled, set_pulled=set_chosen, reward=reward
        )
    return bandit.reward()
    ...
    def multiple_sims(num_sims=50):
        with Pool(10) as pool:
            sim_out = pool.starmap(
                single_sim, [(i, algorithm, probs, horizon) for i in range(num_sims)]
            )
        return sim_out
...
def task3(algorithm, probs, fault, num_sims=50):
    ...
    plt.title("Reward vs Horizon")
    plt.savefig(
        "task3-{}-{}.png".format(algorithm.__name__, time.strftime("%Y%m%d-%H%M%S"))
    )
    plt.clf()
